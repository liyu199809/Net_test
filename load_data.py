...
def load_batch(x, y):
    ...
    for i in range(len(x)):
        img = matplotlib.pyplot.imread(x[i])
        lab = matplotlib.pyplot.imread(y[i])
...
def prepare_data():
    ...
    img = np.array(sorted(glob.glob(r'./dataset/train/img/*.tif')))
    label = np.array(sorted(glob.glob(r'./dataset/train/lab/*.tif')))
    test_img = np.array(sorted(glob.glob(r'./dataset/test/img/*.tif')))
    test_label = np.array(sorted(glob.glob(r'./dataset/test/lab/*.tif')))
